...
const calculatorsHTML = {
    ...
    padua: `<!-- Conteúdo da Calculadora PADUA -->`, // Novo
};
...
// NOVO: HTML da Calculadora de Pádua
calculatorsHTML.padua = `
<div class="calculator-container">
  <h2>Escore de Pádua para Risco de TEV em Pacientes Clínicos</h2>
  <p style="font-size: 0.9em; opacity: 0.8;">Avalia o risco de Tromboembolismo Venoso (TEV) em pacientes clínicos hospitalizados para guiar a necessidade de profilaxia.</p>
  <h3 class="section-header">Fatores de Risco (3 Pontos)</h3>
  <div class="checkbox-group"><input type="checkbox" id="padua-cancer" data-points="3"><label for="padua-cancer">Câncer ativo (local ou metastático, ou quimio/radioterapia nos últimos 6 meses)</label></div>
  <div class="checkbox-group"><input type="checkbox" id="padua-prev-tev" data-points="3"><label for="padua-prev-tev">TEV prévio (excluindo trombose venosa superficial)</label></div>
  <div class="checkbox-group"><input type="checkbox" id="padua-mobility" data-points="3"><label for="padua-mobility">Mobilidade reduzida (restrito ao leito com permissão para ir ao banheiro por ≥3 dias)</label></div>
  <div class="checkbox-group"><input type="checkbox" id="padua-thrombophilia" data-points="3"><label for="padua-thrombophilia">Trombofilia conhecida</label></div>
  <h3 class="section-header">Fatores de Risco (2 Pontos)</h3>
  <div class="checkbox-group"><input type="checkbox" id="padua-trauma" data-points="2"><label for="padua-trauma">Trauma e/ou cirurgia recente (≤1 mês)</label></div>
  <h3 class="section-header">Fatores de Risco (1 Ponto)</h3>
  <div class="checkbox-group"><input type="checkbox" id="padua-age" data-points="1"><label for="padua-age">Idade ≥ 70 anos</label></div>
  <div class="checkbox-group"><input type="checkbox" id="padua-failure" data-points="1"><label for="padua-failure">Insuficiência cardíaca ou respiratória</label></div>
  <div class="checkbox-group"><input type="checkbox" id="padua-stroke" data-points="1"><label for="padua-stroke">IAM ou AVC isquêmico agudo</label></div>
  <div class="checkbox-group"><input type="checkbox" id="padua-infection" data-points="1"><label for="padua-infection">Infecção aguda e/ou doença reumatológica</label></div>
  <div class="checkbox-group"><input type="checkbox" id="padua-obesity" data-points="1"><label for="padua-obesity">Obesidade (IMC ≥ 30 kg/m²)</label></div>
  <div class="checkbox-group"><input type="checkbox" id="padua-hormone" data-points="1"><label for="padua-hormone">Tratamento hormonal em curso</label></div>
  <div class="button-group"><button id="padua-calc-button">Calcular Escore de Pádua</button><button id="padua-reset-button" class="secondary">Reiniciar</button></div>
  <h3>Escore Total: <span id="padua-result"></span></h3>
  <h4>Risco de TEV e Recomendação:</h4>
  <div id="padua-interpretation" style="padding: 10px; background-color: #e9ecef; border-radius: 4px; margin-top: 5px; line-height: 1.5;"></div>
</div>`;
...
mainContainer.innerHTML = `
  <div id="med-calc-menu">
    ...
    <button data-target="padua">Pádua (TEV)</button> <!-- Novo Botão -->
  ...
  <div id="calculator-display-area">
    ...
    <div id="padua-container" class="calculator-content"></div> <!-- Novo Contêiner -->
...
const calculatorsLogic = {
    ...
    // NOVO: Lógica da Calculadora de Pádua
    init_padua: () => {
        calculatorsLogic.getEl('#padua-calc-button')?.addEventListener('click', () => {
            let score = 0;
            mainContainer.querySelectorAll('#padua-container input:checked').forEach(box => {
                score += parseInt(box.dataset.points);
            });
            calculatorsLogic.getEl('#padua-result').innerText = score;
            let interp = "";
            if (score >= 4) {
                interp = `<strong>Alto Risco (${score} pontos):</strong> Recomenda-se profilaxia para TEV (farmacológica ou mecânica), na ausência de contraindicações.`;
            } else {
                interp = `<strong>Baixo Risco (${score} pontos):</strong> Profilaxia para TEV não é indicada de rotina. Medidas gerais como deambulação precoce são encorajadas.`;
            }
            calculatorsLogic.getEl('#padua-interpretation').innerHTML = interp;
        });
        calculatorsLogic.getEl('#padua-reset-button')?.addEventListener('click', () => {
            mainContainer.querySelectorAll('#padua-container input[type="checkbox"]').forEach(c => c.checked = false);
            calculatorsLogic.getEl('#padua-result').innerText = "";
            calculatorsLogic.getEl('#padua-interpretation').innerHTML = "";
        });
    },
